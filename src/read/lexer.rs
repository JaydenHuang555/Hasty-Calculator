use crate::{
    read::error::{LexerError, OperandLexerError},
    token::{Token, operator::executables, parantheses::ParanthesesType},
};
...
pub fn postfix(infix_equation: &str) -> Result<Vec<Token>, LexerError> {
    ...
    for value in infix_equation.chars() {
        ...
        match current_state {
            LexState::Nothing => {}
            LexState::Unknown => {
                return Result::Err(LexerError::UnknownNextLexState(prev_state, value));
            }
                ...
                if prev_state == LexState::BuildOperand {
                    if !builder.is_empty() {
                        match builder.parse::<f64>() {
                            ...
                            Err(_) => {
                                return Result::Err(LexerError::OperandError(
                                    OperandLexerError::InvalidNumericOperandConversion(builder),
                                ));
                            }
                        }
                    }
            ...
            LexState::BuildOperator => {
                    ...
                    match operators.last() {
                        ...
                        Option::Some(peek) => match peek {
                            Token::Operator(executable) => {
                                if executable.prec()
                                    >= crate::token::operator::executable::prec(value)
                                {
                                    buffer.push(*peek);
                                    operators.pop();
                                } else {
                ...
                match executables::match_char_with_executable(value) {
                    ...
                    Option::None => {
                        return Result::Err(LexerError::InvalidOperatorDetectedAsOperator(value));
                    }
                }
            }
        }
